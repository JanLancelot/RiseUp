...
export default function Rosary() {
  ...
  if (loading) {
    return <div style={styles.pageContainer}><p style={styles.statusMessage}>Loading Rosary Videos...</p></div>;
  }

  if (rosaryURLs.length === 0) {
    return <div style={styles.pageContainer}><p style={styles.statusMessage}>The Rosary videos for today are not yet available.</p></div>;
  }
      ...
      <div style={styles.pageContainer}>
        <div style={styles.contentWrapper}>
          <div style={styles.header}>
            <h1 style={styles.title}>The Holy Rosary</h1>
            <h2 style={styles.subtitle}>{dayTitle}</h2>
          </div>
                ...
                return (
                    <div key={url} style={styles.videoCard} onMouseOver={e => e.currentTarget.style.transform = 'translateY(-5px)'} onMouseOut={e => e.currentTarget.style.transform = 'translateY(0)'}>
                        <div style={styles.cardHeader}>
                            <h3 style={styles.cardTitle}>{`Mystery ${label.replace('rosary', '')} of ${rosaryURLs.length}`}</h3>
                        </div>
